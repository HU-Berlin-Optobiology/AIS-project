...
def create_valid_sheet_name(sheet_name):
    invalid_chars = ['\\', '/', '*', '[', ']', ':', '?', '"']
    for char in invalid_chars:
        sheet_name = sheet_name.replace(char, '')
    return sheet_name[:31]  # Excel sheet names must be 31 characters or less
# Construct the file path for the Excel file
excel_file = f"{save_exe}\\{exp_name}_kymo_ana.xlsx"

# Use the `with` statement to ensure the writer is properly closed
try:
    with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
        for sh_n, sh_data in kymo_ana.items():
            sh_n = sh_n.split("\\")[-2]
            sh_n = create_valid_sheet_name(sh_n)
            df = pd.DataFrame.from_dict(sh_data, orient='index')
            df.to_excel(writer, sheet_name=sh_n)
